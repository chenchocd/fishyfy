...
$sql = "INSERT INTO ordenes (usuario_id, fecha_compra, precio_total, hora_recogida) VALUES ('$user_id','$fecha_recogida','$precio', '$hora_recogida')";
$data = query($sql);

// Recupera el ID de la orden recién creada
$orden_id = mysqli_insert_id($conexion); // $conn debe ser tu conexión activa
...
if($data){
    ...
    $subject = "Pedido:";
    $message = 'Este es el resumen de su pedido:
                fecha de recogida: ' . $fecha_recogida . ' hora de recogida: ' . $hora_recogida . ' precio: ' . $precio;
...
if ($data && $orden_id) {
    ...
    // Construye el cuerpo del correo con HTML
    $message = '<p><strong>Este es el resumen de su pedido:</strong></p>';
    $message .= '<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse; width: 100%;">';
    $message .= '<tr><th>Fecha de recogida</th><th>Hora de recogida</th><th>Precio total</th></tr>';
    $message .= '<tr><td>' . $fecha_recogida . '</td><td>' . $hora_recogida . '</td><td>' . $precio . ' €</td></tr>';
    $message .= '</table>';

    $message .= '<p><strong>Detalles de los productos:</strong></p>';
    $message .= '<table border="1" cellpadding="5" cellspacing="0" style="border-collapse: collapse; width: 100%;">';
    $message .= '<tr><th>Nombre</th><th>Descripción</th><th>Cantidad</th><th>Preparación</th></tr>';
